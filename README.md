...
If Flutter debug gets stuck with duplicate runs, service protocol errors, or
`the Dart compiler exited unexpectedly`, use:

```bash
./run_clean_android.sh
```

To target a different Android device:

```bash
./run_clean_android.sh <device-id>
```
# ky-hr-admin-flutter
